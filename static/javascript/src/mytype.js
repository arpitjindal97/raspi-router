...
function LoadHtmlDiv(div_id, html_file) {
    let con = document.getElementById(div_id), xhr = new XMLHttpRequest();
...
function saveButtonClicked() {
    let objecttosend = {
        ...
        BridgeMode: $('[name="bridge"]:checked').val(),
        BridgeMaster: $("#bridge_master").html(),
        NatInterface: $('#nat_int option:selected').val(),
        IpModes: $('[name="ip_mode"]:checked').val(),
        IpAddress: $("#ip_addr").val(),
        SubnetMask: $("#subnet").val(),
        Wpa: $("#wpa_config_area").val(),
        Hostapd: $("#hostapd_config").val(),
        Dnsmasq: $("#dnsmasq_config").val(),
        Info: null
    };
